...
public class TarjanStronglyConnectedComponent {
    ...
    private Map<Vertex<Integer>, Integer> discoveryTime;
    ...
    private void sccUtil(Vertex<Integer> vertex) {
        ...
        for (Vertex<Integer> child : vertex.getAdjacentVertexes()) {
            ...
            if (!visited.contains(child)) {
                sccUtil(child); // visit it and update the low time for that.
            ...
            else if (onStack.contains(child)) { //back edge
                //sets lowTime[vertex] = min(lowTime[vertex], visitedTime[child]);
               lowTime.put(vertex, Math.min(lowTime.get(vertex), discoveryTime.get(child)));
            }
        }
        ...
        //connected component.
        if (discoveryTime.get(vertex).equals(lowTime.get(vertex))) { // if vertex low and disc time is same it means first vertex
